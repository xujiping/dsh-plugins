...
重启目标 DSH profile。侧边栏“新会话”会出现“Claude Code（原生会话）”；默认启用 Claude `--safe-mode`，只公开 `Read`、`Glob`、`Grep`，并用空的严格 MCP 配置隔离所有用户/项目 MCP；绝不启用 `bypassPermissions`。
...
`cordis.patch.yml` 的插件 config 可选字段：`command`、`args`、`tools`、`indexPath`、`securityProfile`、`safeMode`。`safeMode` 默认为 `true`；`args` 不能传 bypass 权限参数。`indexPath` 默认是 `~/.dsh/cc-agent-driver/sessions.json`，只保存 driver 标识、UUID、版本和安全配置，不保存 token 或 Claude 另一个 session ID。
