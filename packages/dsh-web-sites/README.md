...
DSH Web 集中管理自己的网站系统：在界面左侧浮起一个「🌐 网站」入口，展开后是
你的站点列表；点击某个站点，会在 DSH 主面板区域（侧边栏右侧）以 iframe 面板
打开它，带**刷新 / 新标签 / 管理 / 关闭**。站点清单存于 `~/.dsh/sites.yaml`，
支持在界面里可视化增删改。
...
- **左侧触发条**：窗口左缘一个竖向「🌐 网站」胶囊，点击展开/收起站点列表。
- **站点列表**：展开后覆盖 DSH 侧边栏区域，每项显示图标 + 名称 + 地址；
  列表底部有「⚙ 管理站点」入口。
...
- **client 半边**（`lib/client.js`）纯 DOM 注入：所有 UI（触发条 / 列表 /
  iframe 面板 / 管理弹窗）都 `position: fixed` 挂在 `document.body`
  （在 React 树之外），MutationObserver + ResizeObserver 自愈。
